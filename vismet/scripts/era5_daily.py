...
def run(path = '/home/weiglas/Documents/iec/dados/reanalise/era5/daily/',
        ...
        file_year = curDate.date().strftime('%Y')
        
        while(file_year < 2020):
            latitudes = []
            longitudes = []
            lat_dict = {}
            lon_dict = {}
            ...
            try:
                ds = xr.open_dataset(os.path.join(path, file_year + '.grib'), engine='cfgrib')
                file_year += 1
            except FileNotFoundError:
                print('Arquivo não encontrado  ' + path + file_year + '.grib')
                log_file.write('Arquivo não encontrado   ' + path + file_year + '.grib')
            ...
            for p in pixels:
                ...
                for i_time, t in enumerate(time):
                    ...
                    pixel_data = {
                        'date': pandas.to_datetime(t).date().strftime('%Y-%m-%d'),
                    ...
                    print(pandas.to_datetime(t).date().strftime('%d/%m/%Y') + '  ' + str(p.latitude) + ',' + str(p.longitude))
